...
func (pc *PaybackCache) AddTransaction(tx *types.Transaction, receipt *types.Receipt) error {
	...
	if txtype == TxTypeStake {
		stakeAmount = new(big.Int).Set(tx.Value())
	}

	var feeRefund *big.Int
...
// It manages pc.mu internally and never holds the lock across a store call:
// prevEpochState is memoized on blkCtx and fetched outside any lock.
func (pc *PaybackCache) getPaybackData(address common.Address) (currentEpoch idx.Epoch, quotaUsed *big.Int, fullDuration int64, ok bool) {
	// Fast path: read epoch, quota, and memoized prevEpochState under RLock.
	pc.mu.RLock()
	...
	// No store or EVM calls happen inside this section.
	pc.mu.Lock()
...
func (pc *PaybackCache) GetAvailablePaybackByAddress(address common.Address, evm *vm.EVM) *big.Int {
	...
	_, quotaUsed, fullDuration, ok := pc.getPaybackData(address)
	if !ok {
		return payback
	}
