...
// TestSfcV2Patch9_RLPRoundtrip pins the bitfield encoding so a future refactor
// cannot silently change the on-the-wire layout of the SfcV2Patch9 flag. The
// flag rides bit 1<<22 in the Upgrades bitmap.
func TestSfcV2Patch9_RLPRoundtrip(t *testing.T) {
	u := Upgrades{SfcV2Patch9: true}
	...
	require.True(t, decoded.SfcV2Patch9, "SfcV2Patch9 must round-trip as true")
...
// TestSfcV2Patch9_BitfieldDoesNotClashWithOtherFlags confirms the new bit
// (1<<22) is disjoint from every previously assigned bit.
func TestSfcV2Patch9_BitfieldDoesNotClashWithOtherFlags(t *testing.T) {
	...
	require.Equal(t, uint64(1<<22), uint64(sfcV2Patch9Bit), "sfcV2Patch9Bit must be 1<<22 (next free bit after sfcV2Patch8Bit)")
}

// TestSfcV2Patch9_DefaultsFalseOnRealConstructors defends against an accidental
// flip on any network other than the testnet that needs the two-reward-fix
// reflash. Mainnet ships the permanent fix via GetLatestContractBin() at its
// first SfcV2 activation, so it must leave SfcV2Patch9 unset.
func TestSfcV2Patch9_DefaultsFalseOnRealConstructors(t *testing.T) {
	cases := []struct {
		name string
		fn   func() Rules
	}{
	...
	for _, c := range cases {
		...
		t.Run(c.name, func(t *testing.T) {
			require.False(t, c.fn().Upgrades.SfcV2Patch9,
				"%s.Upgrades.SfcV2Patch9 must be false — only VinuChainTestNetRules stages the two-reward-fix reflash", c.name)
		})
	}
}

// TestSfcV2Patch9_TestnetActivated pins that testnet stages the SfcV2Patch9
// reflash (Cycle-164 two-reward-fix bytecode).
func TestSfcV2Patch9_TestnetActivated(t *testing.T) {
	require.True(t, VinuChainTestNetRules().Upgrades.SfcV2Patch9,
		"VinuChainTestNetRules.Upgrades.SfcV2Patch9 must be true to reflash Cycle-164 two-reward-fix bytecode")
}
