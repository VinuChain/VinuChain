...
func newTestEnv(firstEpoch idx.Epoch, validatorsNum idx.Validator) *testEnv {
	...
	genStore := makefakegenesis.FakeGenesisStoreWithRulesAndStart(validatorsNum, utils.ToVC(genesisBalance), utils.ToVC(genesisStake), rules, firstEpoch, 2)
...
func (env *testEnv) callContract(
	...
	res, err := evmcore.NewStateTransition(vmenv, msg, gaspool, nil, nil).TransitionDb()
