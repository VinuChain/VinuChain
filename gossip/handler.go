...
type handler struct {
	...
	peerEventQuota  *peerEventQuota
	peerStreamQuota *peerEventQuota
...
func (h *handler) unregisterPeer(id string) {
	...
	h.peerEventQuota.RemovePeer(id)
	h.peerStreamQuota.RemovePeer(id)
...
func (h *handler) handle(p *peer) error {
	...
	if !useless && !eligibleForSnap(p.Peer) {
		useless = true
		discfilter.Ban(p.ID())
	} else if !useless && !strings.Contains(strings.ToLower(p.Name()), "opera") {
		useless = true
	}
