...
func TestCheckStoredTestnetUpgradeSeals(t *testing.T) {
	...
	current.NetworkID = opera.VinuChainTestNetworkID

	nonTestnet := current
	nonTestnet.NetworkID = opera.VinuChainMainNetworkID
	assert.NoError(checkStoredTestnetUpgradeSeals(nonTestnet, 1, false))
	assert.NoError(checkStoredTestnetUpgradeSeals(current, 2, true),
		"a testnet generated by `opera network new` is not stale public chaindata")

	err := checkStoredTestnetUpgradeSeals(current, idx.Epoch(6118), false)
	assert.Error(err)
	...
	for _, missing := range []struct {
		...
		t.Run(missing.name, func(t *testing.T) {
			...
			err := checkStoredTestnetUpgradeSeals(rules, idx.Epoch(6119), false)
			require.Error(err)
			require.Contains(err.Error(), missing.name)
		})
	}

	assert.NoError(checkStoredTestnetUpgradeSeals(current, idx.Epoch(6119), false))
}
